  ...
  _renderOptions() {
    ...
    let optionNodes = options.map(option => {
      return selected === option.value || option.disabled ? (
        <li key={option.value} className='disabled'>
          {option.label}
        </li>
      ) : (
        <li
          key={option.value}
          onClick={() => this._onSelectOption(option.value)}>
          {option.label}
        </li>
      )
    })
    ...
    if (label) {
      optionNodes.unshift(
        <li key='label-option' className='labelOption'>
          {label}
        </li>,
      )
    }
    ...
    return optionsVisible ? (
      <ul className='optionsList'>{optionNodes}</ul>
    ) : null
  }
